export const FETCH_ERROR = 'FETCH_ERROR'
export const FETCH_LOADING = 'FETCH_LOADING'
export const FETCH_ERROR_CLEAR = 'FETCH_ERROR_CLEAR'

export const SET_USER_FILTER = 'SET_USER_FILTER'
export const SET_TAB_FILTER = 'SET_TAB_FILTER'
export const SET_HISTORY_FILTER = 'SET_HISTORY_FILTER'

export const GET_BOARD_LOADING = 'GET_BOARD_LOADING'
export const GET_BOARD_SUCCESS = 'GET_BOARD_SUCCESS'
export const GET_BOARD_ERROR = 'GET_BOARD_ERROR'

export const GET_BOARD_FILTER = 'GET_BOARD_FILTER'

export const ADD_CARD_LOADING = 'ADD_CARD_LOADING'
export const ADD_CARD_SUCCESS = 'ADD_CARD_SUCCESS'
export const ADD_CARD_ERROR = 'ADD_CARD_ERROR'

export const CHANGE_CARD_LOADING = 'CHANGE_CARD_LOADING'
export const CHANGE_CARD_SUCCESS = 'CHANGE_CARD_SUCCESS'
export const CHANGE_CARD_ERROR = 'CHANGE_CARD_ERROR'

export const DELETE_CARD_SUCCESS = 'DELETE_CARD_SUCCESS'
export const DELETE_CARD_ERROR = 'DELETE_CARD_ERROR'

export const CARD_FILE_LOADING = 'CARD_FILE_LOADING'
export const CARD_FILE_ERROR = 'CARD_FILE_ERROR'

export const ADD_PAYER_LOADING = 'ADD_PAYER_LOADING'
export const ADD_PAYER_ERROR = 'ADD_PAYER_ERROR'

export const DELETE_PAYER_LOADING = 'DELETE_PAYER_LOADING'
export const DELETE_PAYER_ERROR = 'DELETE_PAYER_ERROR'

export const SET_PAYER_LOADING = 'SET_PAYER_LOADING'
export const SET_PAYER_ERROR = 'SET_PAYER_ERROR'

export const GET_CARDS_FILTER = 'GET_CARDS_FILTER'

export const GET_BUDGET_LOADING = 'GET_BUDGET_LOADING'
export const GET_BUDGET_SUCCESS = 'GET_BUDGET_SUCCESS'
export const GET_BUDGET_ERROR = 'GET_BUDGET_ERROR'

export const GET_TRAVEL_LOADING = 'GET_TRAVEL_LOADING'
export const GET_TRAVEL_SUCCESS = 'GET_TRAVEL_SUCCESS'
export const GET_TRAVEL_ERROR = 'GET_TRAVEL_ERROR'

export const CHANGE_TRAVEL_LOADING = 'CHANGE_TRAVEL_LOADING'
export const CHANGE_TRAVEL_SUCCESS = 'CHANGE_TRAVEL_SUCCESS'
export const CHANGE_TRAVEL_ERROR = 'CHANGE_TRAVEL_ERROR'

export const CREATE_TRAVEL_LOADING = 'CREATE_TRAVEL_LOADING'
export const CREATE_TRAVEL_SUCCESS = 'CREATE_TRAVEL_SUCCESS'
export const CREATE_TRAVEL_ERROR = 'CREATE_TRAVEL_ERROR'

export const DELETE_TRAVEL_LOADING = 'DELETE_TRAVEL_LOADING'
export const DELETE_TRAVEL_SUCCESS = 'DELETE_TRAVEL_SUCCESS'
export const DELETE_TRAVEL_ERROR = 'DELETE_TRAVEL_ERROR'

export const GET_USER_LOADING = 'GET_USER_LOADING'
export const GET_USER_SUCCESS = 'GET_USER_SUCCESS'
export const GET_USER_ERROR = 'GET_USER_ERROR'

export const UPDATE_USER_LOADING = 'UPDATE_USER_LOADING'
export const UPDATE_USER_SUCCESS = 'UPDATE_USER_SUCCESS'
export const UPDATE_USER_ERROR = 'UPDATE_USER_ERROR'

export const USER_AVATAR_LOADING = 'USER_AVATAR_LOADING'
export const USER_AVATAR_ERROR = 'USER_AVATAR_ERROR'

export const GET_TRAVELS_FILTER = 'GET_TRAVELS_FILTER'

export const SEARCH_CONTACT_LOADING = 'SEARCH_CONTACT_LOADING'
export const SEARCH_CONTACT_SUCCESS = 'SEARCH_CONTACT_SUCCESS'
export const SEARCH_CONTACT_ERROR = 'SEARCH_CONTACT_ERROR'

export const INVITE_CONTACT_LOADING = 'INVITE_CONTACT_LOADING'
export const INVITE_CONTACT_SUCCESS = 'INVITE_CONTACT_SUCCESS'
export const INVITE_CONTACT_ERROR = 'INVITE_CONTACT_ERROR'

export const UPDATE_CONTACTS_ERROR = 'UPDATE_CONTACTS_ERROR'
export const UPDATE_CONTACTS_SUCCESS = 'UPDATE_CONTACTS_SUCCESS'

export const CLEAR_CONTACTS_SEARCH = 'CLEAR_CONTACTS_SEARCH'
export const CLEAR_ERROR_SEARCH = 'CLEAR_ERROR_SEARCH'

export const GET_INVITED_EMAIL_LOADING = 'GET_INVITED_EMAIL_LOADING'
export const GET_INVITED_EMAIL_SUCCESS = 'GET_INVITED_EMAIL_SUCCESS'
export const GET_INVITED_EMAIL_ERROR = 'GET_INVITED_EMAIL_ERROR'

export const EMAIL_CONFIRMATION_LOADING = 'EMAIL_CONFIRMATION_LOADING'
export const EMAIL_CONFIRMATION_SUCCESS = 'EMAIL_CONFIRMATION_SUCCESS'
export const EMAIL_CONFIRMATION_ERROR = 'EMAIL_CONFIRMATION_ERROR'

export const REGISTRATION_LOADING = 'REGISTRATION_LOADING'
export const REGISTRATION_SUCCESS = 'REGISTRATION_SUCCESS'
export const REGISTRATION_ERROR = 'REGISTRATION_ERROR'

export const LOGIN_LOADING = 'LOGIN_LOADING'
export const LOGIN_SUCCESS = 'LOGIN_SUCCESS'
export const LOGIN_ERROR = 'LOGIN_ERROR'

export const LOGOUT_SUCCESS = 'LOGOUT_SUCCESS'
export const LOGOUT_ERROR = 'LOGOUT_ERROR'
export const UNAUTHORIZED = 'UNAUTHORIZED'

export const PASSWORD_CHANGE_REQUEST_LOADING = 'PASSWORD_CHANGE_REQUEST_LOADING'
export const PASSWORD_CHANGE_REQUEST_SUCCESS = 'PASSWORD_CHANGE_REQUEST_SUCCESS'
export const PASSWORD_CHANGE_REQUEST_ERROR = 'PASSWORD_CHANGE_REQUEST_ERROR'

export const PASSWORD_CHANGE_LOADING = 'PASSWORD_CHANGE_LOADING'
export const PASSWORD_CHANGE_SUCCESS = 'PASSWORD_CHANGE_SUCCESS'
export const PASSWORD_CHANGE_ERROR = 'PASSWORD_CHANGE_ERROR'

export const GET_EMAIL_PASSWORD_CHANGE_LOADING =
   'GET_EMAIL_PASSWORD_CHANGE_LOADING'
export const GET_EMAIL_PASSWORD_CHANGE_SUCCESS =
   'GET_EMAIL_PASSWORD_CHANGE_SUCCESS'
export const GET_EMAIL_PASSWORD_CHANGE_ERROR = 'GET_EMAIL_PASSWORD_CHANGE_ERROR'

export const FEEDBACK_LOADING = 'FEEDBACK_LOADING'
export const FEEDBACK_SUCCESS = 'FEEDBACK_SUCCESS'
export const FEEDBACK_ERROR = 'FEEDBACK_ERROR'
