...
const authError = {
   ...
   notFoundError: createError(
      400,
      'Не получилось подтвердить почту. Возможно, ссылка устарела - проверьте почтовый ящик или запросите письмо еще раз',
      {
         type: ErrorType.ERROR,
      }
   ),
...
const commonError = createError(
   500,
   'Похоже, возникли проблемы с сервером.\n' +
...
const success = {
   ...
   forgotSuccess: {
      message:
         'Пароль сброшен, ссылка отправлена на почту – проверьте «входящие» (и «спам», если письмо долго не приходит 🤔)',
      type: ErrorType.SUCCESS,
   },
   changePasswordSuccess: {
      message: 'Пароль изменен (если забудете его снова – вы знаете, что делать 😎)',
      type: ErrorType.SUCCESS,
   },
   inviteSuccess: {
